...
const CustomizerSystem = () => {
	...
	const { themeType, themeColor, navStyle, layoutType, isDirectionRTL } = useSelector(({ settings }: any) => settings);
	...
	const onNavStyleChanges = (navStyle: any) => {
		...
		return (
			<CustomScrollbars className="gx-customizer">
				...
				{/* <div className="gx-customizer-item">
					...
					<Form.Item label="Layout Orientation (RTL)" className={isDirectionRTL ? 'gx-mr-0' : 'gx-ml-4'}>
	...
	function onChangeDirection(checked: any) {
	...
	const handleLayoutTypes = (layoutType: any) => {
	...
	const getLayoutsTypes = (layoutType: any) => {
		return (
	...
	const getNavStyles = (navStyle: any) => {
		return (
			<ul className="gx-nav-option gx-list-inline">
				...
				{/* <li>
					<span onClick={() => onNavStyleChanges(NAV_STYLE_BELOW_HEADER)} className={`gx-pointer ${navStyle === NAV_STYLE_BELOW_HEADER && 'active'}`}>
						<img src={'/assets/images/layouts/below header.png'} alt="below header" />
					</span>
				</li>

				<li>
					<span onClick={() => onNavStyleChanges(NAV_STYLE_ABOVE_HEADER)} className={`gx-pointer ${navStyle === NAV_STYLE_ABOVE_HEADER && 'active'}`}>
	...
	return (
		<>
			<Drawer
				...
				width={isCustomizerOpened ? 0 : 400}
